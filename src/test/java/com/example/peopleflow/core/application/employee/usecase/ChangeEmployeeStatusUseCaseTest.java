...
public class ChangeEmployeeStatusUseCaseTest {
    ...
    @Test
    @DisplayName("Creating an employee use case returns valid id and status")
    public void employeeCreationUseCase_withValidRequest_createsEmployeeWithAddedStatus() {

        //GIVEN
        final var expectedId = 1;
        final var expectedStatus = EmployeeStatus.ACTIVE.toString();

        final var employeeStatusRequestModel = constructEmployeeStatusRequestModel();
        final var employeeCreationRequestModel = constructEmployeeCreationRequestModel();
        ...
        // WHEN
        final var response = employeeUseCase.changeEmployeeStatus(employeeStatusRequestModel);

        // THEN
        verify(employeeRepository, new Times(1)).upsert(employeeModelCapture.capture());
        verify(employeeRepository, new Times(1)).findById(idCapture.capture());
        ...
        final var idOnRepo = idCapture.getValue();

        assertThat(employeeOnRepo.getStatus()).isEqualTo(EmployeeStatus.ACTIVE);
        ...
        assertThat(employeeOnConverter.getStatus()).isEqualTo(EmployeeStatus.ACTIVE);
        ...
        assertThat(idOnRepo).isEqualTo(expectedId);
    ...
    private EmployeeStatusRequestModel constructEmployeeStatusRequestModel() {
    ...
    private EmployeeCreationRequestModel constructEmployeeCreationRequestModel() {
